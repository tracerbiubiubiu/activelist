...
> - **继续有效（数据模型层）**：§4 技术栈、§5.1/§5.2（数据/元数据模型）、§6.1–6.5（Registry/Repository/Validation/Schema 演进/软删除状态机）、§6.7–6.9（查询安全/错误码/API）、§10（并发控制）、§15（安全）。
...
> | 认证/鉴权 | **activelist 完全不管**，zhuzhao 网关统一鉴权 + X-Operator（§19.2 不变） |
> | 查询 | **仅按唯一 id 分页 + 创建时间倒序**；无过滤、无排序参数、无聚合（§6.7 动态查询引擎整块砍掉） |
> | 量级 | 百万行以内 |
> | 可靠性 | 高——因存**敏感高危数据**（可靠性由安全驱动，非规模驱动）；存储加密**已拍板不做**（2026-09-03：内网 + 日志脱敏 + 审计一期不落字段值覆盖当前风险；跨网/合规变化再启）；日志脱敏仍需 |
...
> - **导入幂等 = 全量替换（非 upsert）**：单事务内 DELETE 该类型全表（**含软删行，替换不可恢复**）→ 按文件插入（**保留源 id**、`version` 重置 1、`created_at` 沿用文件、`updated_at`/`updated_by` = 导入时刻/操作者）→ `setval` 序列至 max(id)。重导同一文件结果一致即幂等；**不要求每类型声明业务唯一键**（原 upsert 方案作废）。导出格式必须含 `id` 与 `created_at`。
...
| **activelist API** | 不做认证（内网信任模式，由 zhuzhao 网关鉴权） |
